...
// interface 정의
export interface RefineAmount {
  [material: string]: number;
}

export interface RefineBreath {
  [breathName: string]: [number, number];
}
...
export interface RefineDataTier {
  [refineLevel: number]: RefineTableData;
}

export interface RefineDataGrade {
  [grade: string]: RefineDataTier;
}

export interface RefineDataType {
  [type: string]: RefineDataGrade;
}

export const refineData: RefineDataType = {
  armor: {
    t3_1250: {
      3: {
        baseProb: 0.5,
...
// 이 함수는 장비의 종류(itemType), 등급(itemGrade), 목표 강화 수치(refineTarget), 연구 적용 여부(applyResearch), 하이퍼 익스프레스 적용 여부(applyHyperExpress)를 받아서
// 해당 강화 구간의 확률, 비용 감소, 골드 감소, 정인 배수, 재료 소모량, 기본 확률, 숨결 테이블 등을 계산하여 반환하는 함수입니다.
// 주로 강화 시뮬레이션이나 비용 계산 등에 사용됩니다.
export function getRefineTable(
  ...
  // t3_1250 구간별 기본 추가 확률, 비용/골드 감소 설정
  ...
  // 하이퍼 익스프레스 적용 시 확률/비용/골드 감소 등 특수 처리
  if (applyHyperExpress) {
    // 2024 여름, 슈퍼 모코코 익스프레스
  ...
  // 결과 반환: 추가 확률, 정인 배수, 재료 소모량(골드/재료 각각 감소율 적용), 기본 확률, 숨결 테이블
  return {
    additionalProb,
    janginMultiplier,
